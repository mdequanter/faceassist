...
def play_qr_click(duration_ms: int = 70, frequency: int = 1200, volume: int = 100) -> None:
    try:
        ...
        # Raspberry Pi 5 + Jieli UACDemoV1.0 USB audio is stable with WAV playback,
        # but may fail with raw ALSA streaming (Unknown error 524).
        wav_path = None
        try:
...
def scale_wav_volume_in_place(wav_path: str, volume: int) -> None:
    """Scale a 16-bit PCM WAV file in place. If scaling fails, leave the file unchanged."""
    ...
    try:
        ...
        scaled = pcm.astype(np.int16).tobytes()

        with wave.open(wav_path, "wb") as wf:
            wf.setparams(params)
            wf.writeframes(scaled)
    except Exception as exc:
        print(f"[WARNING] Could not scale WAV volume: {exc}", flush=True)
...
def piper_say(
    ...
    # Raspberry Pi 5 + Jieli UACDemoV1.0 USB audio:
    # WAV playback works reliably, while raw streaming to aplay can fail with
    # "audio open error: Unknown error 524". Therefore Piper writes a temporary
    # WAV file and aplay plays that file.
    wav_path = None
    ...
    try:
        ...
        scale_wav_volume_in_place(wav_path, volume)
